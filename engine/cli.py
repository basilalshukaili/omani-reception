...
@cli.command(help="Interactive chat REPL (lands in P2).")
@_business_option()
@_log_level_option()
def chat(business_id: str, log_level: str) -> None:
    """Stub for the interactive REPL — real implementation in P2."""
    configure_logging(level=log_level)
    log = get_logger("engine.cli.chat")
    log.info("chat.stub", business_id=business_id)
    click.echo("chat REPL coming in P2")
